...
from src.config import (
    ...
    CHUNK_SIZE, CHUNK_OVERLAP, SIMILARITY_THRESHOLD, RETRIEVAL_MODE
)
...
def run_benchmark(limit: int = None):
    ...
    print(f"  Chunking: size={CHUNK_SIZE}, overlap={CHUNK_OVERLAP} | Retrieval: {RETRIEVAL_MODE} (threshold={SIMILARITY_THRESHOLD})")
    print("=" * 70)
    ...
    if evaluator.judge_available:
        print("LLM-as-Judge faithfulness evaluation: ENABLED")
    else:
        print("LLM-as-Judge faithfulness evaluation: offline (deterministic clause-support heuristic)")
    ...
    for row in tqdm(questions, desc="Evaluating Questions"):
        ...
        eval_rag_k3 = evaluator.evaluate_sample(
            ...
            category=category,
            in_corpus=in_corpus,
            context=res_rag_k3["retrieved_context"] if res_rag_k3["num_chunks_retrieved"] > 0 else ""
        )
        ...
        eval_rag_k5 = evaluator.evaluate_sample(
            ...
            category=category,
            in_corpus=in_corpus,
            context=res_rag_k5["retrieved_context"] if res_rag_k5["num_chunks_retrieved"] > 0 else ""
        )
        ...
        eval_rag_loose = evaluator.evaluate_sample(
            ...
            category=category,
            in_corpus=in_corpus,
            context=res_rag_loose["retrieved_context"] if res_rag_loose["num_chunks_retrieved"] > 0 else ""
        )
        ...
        record = {
            ...
            "rag_k3_chunks_retrieved": res_rag_k3["num_chunks_retrieved"],
    ...
    def rows_for(prefix):
        return [
            {
                "hallucinated": r[f"{prefix}_hallucinated"],
                "factual_correctness": r[f"{prefix}_correctness"],
                "faithfulness": r[f"{prefix}_faithfulness"],
                "f1_score": r[f"{prefix}_f1"],
                "helpfulness": 5 if r[f"{prefix}_correct_score"] == 1 else (3 if r[f"{prefix}_correct_score"] == 0.5 else 1)
            }
            for r in results_data
        ]

    summary_metrics = {
        "Baseline (No RAG)": evaluator.compute_aggregate_metrics(rows_for("baseline")),
        "RAG (Top-3 Strict)": evaluator.compute_aggregate_metrics(rows_for("rag_k3")),
        "RAG (Top-5 Strict)": evaluator.compute_aggregate_metrics(rows_for("rag_k5")),
        "RAG (Top-3 Loose)": evaluator.compute_aggregate_metrics(rows_for("rag_loose"))
    }

    # Save machine-readable summary alongside the plots
    summary_payload = {
    ...
    print(f"\n>> RESULT: Hallucination rate went from {base_hr:.1f}% (Baseline) to {rag_hr:.1f}% (RAG Top-3 Strict)")
    print(f">> Absolute reduction: {base_hr - rag_hr:.1f} percentage points ({reduction:.1f}% relative).")
    print(f">> Full results saved to: {RESULTS_FILE}")
    print(f">> Summary saved to: {SUMMARY_FILE}")
    print(f">> Plots generated in: {PLOTS_DIR}\n")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run the RAG hallucination benchmark.")
    parser.add_argument("--limit", type=int, default=None, help="Only evaluate the first N questions (for quick tests).")
